/* First version of the Date class program.  Uses global arrays.
*/
#include <iostream>
#include <vector>
#include <string>
...
{  public: // defines the interface-  how the private data is exposed for reading and writing
   ...
   private: // This data is encapsulated (only accessible through the interface)
...
{  assert(m >= 1 && m <= 12 && d >= 1 && d <= mdays[m]);       // assert requires the <cassert> library
   day = d;
   month = m;
}
